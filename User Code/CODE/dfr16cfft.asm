...
;
; Description:  56852 Assembly module for Complex FFT
;
; Modules
;    Included:  Fdfr16CFFT
;
...
FFT_OUTPUT_IS_BITREVERSED         equ   8 ;Default to normal (linear)
    ...
    DEFINE    ScaleBF        'X:(SP-2)'   ;Counter to count the number of
                                          ...
                                          ;in BFP method.
    DEFINE    pZ             'X:(SP-4)'   ;For storing pointer to the output array
...
;---------------------------------
    ...
    adda #Scratch_Size,sp                 ;Update stack pointer by adding
                                          ;Scratch locations needed for FFT.
...
;    move.w  r4,pZ                         ;Storing pointer to the output array
    StoreRx r4,pZ                         ;Storing pointer to the output array
    tfra    r3,r1                         ;storing r3
    nop
    nop
    nop
    nop
...
;--------------------------------------------------------
    ...
    cmp.w   #FAIL,y0
    bne     FFT_Computation               ;Bit reverse passed, go to FFT_Computation.
    moveu.w old_omr,omr                   ;Restore previous OMR value.
    moveu.w #-Scratch_Size,n              ;Restore Stack pointer.
    adda    n,sp
    move.l  x:(sp)-,r5
    move.l  x:(sp)-,d
    move.l  x:(sp)-,d2
...
;    moveu.w pZ,r4                         ;getting the pointer to the output array in r4
    LoadRx  pZ,r4                         ;getting the pointer to the output array in r4
    ...
    adda    n,r0
    deca    r0                            ;Adjust r0 for first pass
...
_first_pass
    ...
    adda    n,r3                          ;r3 -> first Br of second pass
                                          ;Set counter for no. of passes
    move.w  x:(r5+Offset_No_of_Stages),c
    sub.w   #2,c

_second_pass
    LoadRx  x:(r5+Offset_Twiddle),r0      ;twiddle fac. ,twiddle fac.
                                          ;stored in bit reversed fashion
    asra    n                             ;Halving n =n/2,butterflies per group is n
    ...
    move.w  x:(r0)+,y0                    ;y0=Wr,x0=Br,r0 ->Wi, r3 ->Bi
    move.w  x:(r3)+,x0
    move.w  x:(r0)+,y1                    ;y1=Wi, r0 -> next Wr
                                          ;(in bit reversed order)
;    move.w  r0,d                          ;Save twiddle factor pointer in d register
    StoreRx r0,d                          ;Save twiddle factor pointer in d register
...
_end_group
    ...
    moveu.w b,n3
    dec.w   c
    tst.w   c                             ;Test the pass counter for Zero
...
_end_groupBF
    ...
    asl     c                             ;Double the no of groups for next
                                          ;pass
    adda    n,r3                          ;adjust r3-> first Br of the next pass
...
_nscdown                                  ;No scale down mode
    ...
    move.w  x:(r0)+,y0                    ;y0=Wr,x0=first Br of the
    move.w  x:(r3)+,x0                    ;group. r0->Next Wr. r3->first
                                          ;Bi of the group.
    ...
    move.w  x:(r3)-,x0                    ;y1=Wi, b=WrBr, x0=Bi, r3->Br
                                          ;r0->Wi
...
_end_group2
    ...
    moveu.w d1,n
    tfra    r4,r3                         ;r3 -> 1st Ar, for start of each pass
    tfra    r3,r1                         ;r1 -> 1st Ar of next pass
    asl     c                             ;Double the no of groups for next pass
    adda    n,r3                          ;adjust r0, r0 -> first Br of the next pass
...
_lpass                                    ;data scanning for the last pass
    ...
    move.w  #0,x0                         ;x0= 0; smallest positive number
    move.w  x:(r0)+,a                     ;a= first Ar, r3 -> First Ai
...
_lnscdown                                 ;Last pass no scale down
    ...
    move.w  x:(r3)+,x0                    ;x0=Br,r3->Bi
    mpy     x0,y0,b      x:(r0)+,y1       ;b=WrBr, y1=Wi,r0->Wi
                                          ;r3->Br
    move.w  x:(r3)-,x0                    ;x0=Bi
    macr    x0,y1,b      x:(r1)+n,a       ;b=WrBr+WiBi, a=Ar, r1->Ar
    add     a,b                           ;Find Cr in b, b=Ar+WrBr+WiBi,
    asl     a            b,x:(r1)+        ;a=2Ar, store Cr, r1->Ai
...
;    moveu.w pZ,r4                         ;Copy pointer to the output buffer in r4
    LoadRx  pZ,r4                         ;Copy pointer to the output buffer in r4
    moveu.w x:(r5+Offset_n),n             ;n=number of points in FFT
    tfra    r4,r2                         ;r2 output buffer
    tfra    r1,r3                         ;r3 -> 1st Ar of first pass
    adda    n,r3                          ;r3 -> 1st Br of 1st pass
    tfra    r2,r0                         ;r0 -> 1st Br of first pass
...
;--------------------------------------------------------------
    ...
    move.w  x:(r2+Offset_n),y0            ;y0 = N, the size of FFT.
    tfra    r4,r2                         ;Input pointer
    tfra    r2,r3                         ;Output pointer
    jsr     Fdfr16Cbitrev_                ;Call bit reverse function.
                                          ;Result is returned in y0.
    cmp.w   #FAIL,y0
    bne     no_bitrev_output              ;Bit reverse passed, go to FFT.
    moveu.w old_omr,omr                   ;Restore previous OMR value.
